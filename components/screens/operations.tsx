...
/* ===== Threat Detection ===== */
export function ThreatDetectionScreen({  onOpenAlert  }: ScreenProps) {
...
export function IncidentsScreen({  onOpenAlert  }: ScreenProps) {
  ...
  const rows = useMemo(() =>
    QUEUE.slice(0, 8).map((q, i) => ({
      ...
      cells: Array.from({ length: 24 }, (_, h) => {
        const intensity = Math.max(0, Math.sin((h - 4 + i) / 24 * Math.PI) * (0.7 + seededRandom(i * 24 + h) * 0.5));
          ...
          <div key={row.who} style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: 6, alignItems: 'center' }}>
            ...
            <div className="heat">
              {row.cells.map((cell, h) => (
                <div key={h} className="heat__cell" style={{ background: cell.bg, opacity: 0.4 + cell.intensity * 0.6 }} title={`${h}:00 — ${Math.round(cell.intensity * 20)} alerts`}></div>
              ))}
            </div>
          </div>
        ))}
...
function FraudLossChart() {
  const bars = useMemo(() =>
    Array.from({ length: 30 }, (_, i) => ({
      x: i * (800 / 30) + 6,
      actual: 30 + Math.sin(i / 3) * 14 + seededRandom(i + 100) * 10,
      prevented: 60 + Math.sin(i / 4) * 22 + seededRandom(i + 200) * 16,
    })),
  []);
        ...
        <svg viewBox="0 0 800 160" width="100%" height="160" preserveAspectRatio="none" role="img" aria-label="Bar chart of fraud loss over the last 30 days: actual loss averaging $142K per day versus prevented loss averaging $2.1M per day; prevention rate 93.7%.">
          {bars.map((bar, i) => (
            <g key={i}>
              ...
              <rect x={bar.x} y={160 - bar.actual} width={(800 / 30) - 4} height={bar.actual} fill="var(--sev-critical)" />
            </g>
          ))}
